	...
	public function admin_bar_link() {
		...
		$wp_admin_bar->add_menu(
			array(
				'parent' => 'site-name',
				'id'     => 'wordpress-reset',
				'title'  => \esc_html__( 'Reset Site', 'wp-reset' ),
		...
		/* <![CDATA[ */
			jQuery('#wordpress_reset_submit').click(function(){
				if ( 'reset' === jQuery('#wordpress_reset_confirm').val() ) {
					var reset = confirm( '<?php esc_html_e( 'This action is not reversable. Clicking OK will reset your database back to the defaults. Click Cancel to abort.', 'wp-reset' ); ?>' );
					if ( reset ) {
